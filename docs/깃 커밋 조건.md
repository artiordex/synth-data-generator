...
2. `코드주석작성지침.md`에 규정된 공통 주석 규칙이 유지되도록 강제함.
...
### Gate 3. `코드주석작성지침.md` 준수 검증
코드 주석의 일관성과 인코딩 깨짐 방지를 위해 `코드주석작성지침.md` 규격을 충족해야 한다.
1. **파일 헤더 박스 의무화**:
...
| **`docs`** | 문서 또는 주석의 추가·수정 (`코드주석작성지침.md` 준수 주석 반영 등) |
...
| **[좋은 예]** | `docs(comments): 코드주석작성지침.md에 따른 전 소스 코드 헤더 및 함수 주석 전수 반영` |
...
- **대상 파일**:
  - `코드주석작성지침.md` 표준 헤더 및 `def` 주석이 반영된 전 소스 코드 파일
  - `코드주석작성지침.md`, `깃 커밋 조건.md`, `저장소작업지침.md`
- **커밋 메시지 예시**:
  ```bash
  git commit -m "docs(comments): 코드주석작성지침.md에 따른 전체 소스 주석 표준화 반영"
  ```
...
# 주석 및 문서 관련 스테이징
git add docs/코드주석작성지침.md "깃 커밋 조건.md" apps/ packages/ tests/ scripts/
git commit -m "docs(comments): 코드주석작성지침.md에 따른 전체 파일 주석 표준화 반영"
```
