...
연번을 앞에 둔다. 합성 데이터 파일도 실제 `docs` 최종본 기준에 맞춰
`1. 고등학생 진로수업 경험과 진로정보 인식_세종.xlsx` 형식으로 저장한다.
일괄 ZIP에서는 처리 순서 기준으로 `01.`, `02.`처럼 두 자리 번호를 붙여
`원본데이터_세종`, `합성데이터_세종`, `심의자료_세종` 폴더 아래에 모은다.
