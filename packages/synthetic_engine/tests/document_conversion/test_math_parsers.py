...
# 테스트용 가상 ZIP 아카이브 인메모리 스트림을 생성함
...
# MathIR 직렬화 시 계약 스키마 및 리소스 매니페스트 유지 동작을 테스트함
...
# DOCX 인라인 수식과 일반 텍스트의 혼합 구조 및 원본 XML 보존을 검증함
...
# DOCX 디스플레이 분수 수식이 구조화된 LaTeX로 변환되는지 검증함
...
# DOCX 표 셀 내부의 수식이 인라인 콘텐츠로 유지되는지 검증함
...
# 미지원 OMML 수식이 검토 대상으로 보존되는지 검증함
...
# 복수 DOCX 수식 객체가 고유한 소스 참조 경로를 가지는지 검증함
...
# HWPX 수식이 원본 스크립트와 XML 대체 리소스를 보존하는지 검증함
...
# HWPX OLE 객체가 수식으로 오인되지 않고 미지원 레코드로 보존되는지 검증함
...
# HWP 5 EqEdit 수식이 스크립트 및 원본 바이너리 레코드를 보존하는지 검증함
...
# 유효하지 않은 HWP 5 EqEdit 레코드가 비어있지 않은 대체 리소스로 유지되는지 검증함
...
# OCR 이미지의 명시적 수식 후보가 바운딩 박스 및 검토 상태를 유지하는지 검증함
...
# PDF 수식 유사 텍스트 스팬이 미검증 후보로 안전하게 보존되는지 검증함
