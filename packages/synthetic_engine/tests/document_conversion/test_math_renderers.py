...
# 테스트용 DocumentIR 인스턴스를 간편하게 생성함
...
# HTML 렌더러가 인라인 LaTeX 및 디스플레이 MathML을 올바르게 렌더링하는지 검증함
...
# HTML 렌더러가 수식 폴백 이미지 및 시각적 검토 뱃지를 포함하는지 검증함
...
# 잘못된 MathML 입력 시 검토 상태로 표시되고 스크립트가 차단되는지 검증함
...
# HTML 복합 표 렌더링 시 가로 스크롤 및 셀 최소 너비가 보장되는지 검증함
...
# 마크다운 렌더러가 검증된 LaTeX와 검토용 폴백 HTML을 생성하는지 검증함
...
# 마크다운 렌더러에서 복합 표가 깨짐 방지를 위해 HTML 폴백으로 렌더링되는지 검증함
...
# 웹 미리보기 CSS가 표 스크롤, 수식 폰트 및 검토 뱃지 스타일을 정의하는지 검증함
