...
# OMML 수식 노드를 MathIR 객체로 변환하고 검토 상태를 기록함
...
# WordprocessingML 노드에서 지정된 태그의 val 속성값을 추출함
...
# twips 단위 수치를 pt 단위 부동소수점으로 변환함
...
# 인라인 항목 목록에서 캐시된 텍스트 문자열을 결합 추출함
...
# 필드 명령 구문과 캐시 텍스트로 FieldIR 객체를 구성함
...
# 복합 Word 필드 제어 토큰 쌍을 결합하여 FieldIR 구조를 정규화함
