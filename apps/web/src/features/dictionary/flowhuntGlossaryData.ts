...
export const GLOSSARY_DATA: GlossaryItem[] = [
  {
    ...
    "description": "특정 개인의 데이터 포함 여부를 어떠한 보조 정보를 결합하더라도 통계적으로 판별할 수 없도록 수학적으로 증명된 프라이버시 보호 모델입니다. 라플라스 메커니즘이나 가우시안 노이즈를 데이터 합성 또는 통계 집계 과정에 주입하며, 프라이버시 예산(Epsilon, ε)을 통해 데이터의 안전성과 유용성(Utility) 간의 균형을 엄밀히 제어합니다. ε 값이 작을수록 강력한 익명성을 보장하며 식약처 임상·보건의료 공공데이터 개방 가이드라인의 핵심 기준입니다.",
    "url": "",
    "category": "보안 & 프라이버시",
    "initial": "ㅊ"
  },
  {
    ...
    "description": "프랑스 국립 컴퓨터과학연구소(Inria)와 국제 프라이버시 연구진이 개발하고 EU GDPR 제29조 작업반 기준을 충족하는 글로벌 표준 익명화 위험 정량 평가 도구입니다. 합성 데이터셋을 대상으로 세 가지 적대적 공격 시뮬레이션을 수행합니다: ① 단일식별 위험(Singling-out), ② 연결성 위험(Linkability), ③ 속성추론 위험(Inference). 각 공격 위험도가 기준치(통상 0.05~0.10 이하)를 충족할 때 안전한 합성데이터로 공인 배포됩니다.",
    "url": "",
    "category": "보안 & 프라이버시",
    "initial": "A"
  },
  {
    "id": "core-003",
    "name": "CTGAN (Conditional Table GAN with Gradient Penalty)",
    "description": "MIT Data to AI Lab에서 개발한 복합 정형(Tabular) 데이터 전용 딥러닝 합성 신경망 모델입니다. 일반 GAN이 정형 데이터에서 겪는 불균형 범주형 변수 처리 실패 및 다봉(Multi-modal) 연속형 수치 변수의 모달 붕괴(Mode Collapse) 문제를 해결하기 위해, 변분 가우시안 혼합 모델(VGM)을 통한 모드별 정규화, 조건부 생성자(Conditional Generator), 팩(Pac) 배치 판별자 및 Wasserstein 거리 손실함수(WGAN-GP)를 적용하여 의료·임상 시험 데이터와 같은 고차원 불균형 테이블을 사실적으로 재현합니다.",
    "url": "",
    "category": "LLM & 생성형 AI",
    "initial": "C"
  },
  {
    ...
    "description": "정형 데이터의 복합 확률 분포를 효율적으로 학습하는 변분 오토인코더 기반 합성 데이터 생성 신경망입니다. 인코더는 입력 레코드를 연속적인 잠재 공간(Latent Space)의 평균(μ)과 분산(σ)으로 압축 표현하고, 디코더는 잠재 벡터로부터 원본의 결합 확률 분포를 따르는 새로운 데이터를 복원·생성합니다. CTGAN 대비 학습 수렴 속도가 매우 빠르고 안정적이며, 중간 규모의 수치형 중심 데이터셋에서 뛰어난 재현 충실도를 나타냅니다.",
    "url": "",
    "category": "LLM & 생성형 AI",
    "initial": "T"
  },
  {
    "id": "core-005",
    "name": "가우시안 코퓰라 (Gaussian Copula Synthesizer)",
    "description": "각 변수의 개별 한계 분포(Marginal Distribution) 추정과 다변량 변수 간의 비선형 상관관계 구조(Copula Dependency)를 수학적으로 분리하여 결합 누적 확률 분포(Joint CDF)를 모델링하는 고속 통계 합성 알고리즘입니다. 딥러닝 모델 대비 CPU 연산만으로 수만 행을 수 초 이내에 초고속 합성할 수 있으며, 수치형 변수 간의 피어슨/스피어만 상관계수를 엄밀하게 보존하므로 통계적 가설 검정 및 회귀 분석용 합성 데이터 생성에 매우 효과적입니다.",
    "url": "",
    "category": "머신러닝 & 딥러닝",
    "initial": "ㄱ"
  },
  {
    ...
    "description": "쿨백-라이블러 발산(KLD)의 비대칭성과 무한대 발산 문제를 보완하여 두 확률 밀도 함수 간의 통계적 거리(0~1)를 대칭적으로 측정하는 지표입니다. 원본 데이터와 합성 데이터의 각 컬럼별 분포를 비교할 때 사용되며, 0에 가까울수록 원본과 합성 분포가 완벽하게 일치함을 뜻합니다. 플랫폼에서는 직관적인 품질 평가를 위해 `분포 유사도 = (1.0 - JSD) × 100%`로 환산하여 리포트에 제공합니다.",
    "url": "",
    "category": "머신러닝 & 딥러닝",
    "initial": "J"
  },
  {
    "id": "core-007",
    "name": "2-표본 콜모고로프-스미르노프 검정 (2-Sample KS Test)",
    "description": "원본 연속형 수치 데이터와 합성 데이터가 동일한 연속 확률 분포에서 추출되었는지를 비모수적(Non-parametric)으로 검증하는 표준 통계 가설 검정입니다. 두 데이터셋의 경험적 누적 분포 함수(ECDF) 간의 최대 절대 수직 거리인 KS 통계량(D)과 이에 대응하는 p-value를 산출합니다. 유의수준 0.05 기준 p-value > 0.05인 경우 '원본과 합성 데이터의 분포가 통계적으로 유의미한 차이가 없다(귀무가설 채택)'로 판정되어 합성 품질의 무결성을 과학적으로 입증합니다.",
    "url": "",
    "category": "머신러닝 & 딥러닝",
    "initial": "0-9"
  },
  {
    "id": "core-008",
    "name": "카이제곱 독립성 및 적합도 검정 (Chi-Square Test)",
    "description": "범주형 및 이산형 변수(예: 성별, 부작용 중증도 등급, 투약 그룹 등)의 원본 관측 빈도와 합성 데이터의 기대 빈도 간 일치도를 검증하는 통계 기법입니다. 카이제곱 통계량과 p-value를 산출하여 범주 간 상대적 비율 및 속성 간 교차 의존성이 왜곡 없이 보존되었는지를 정밀하게 판별하며, 통계적으로 유의미한 빈도 왜곡 발생 여부를 사전 감지합니다.",
    "url": "",
    "category": "머신러닝 & 딥러닝",
    "initial": "ㅋ"
  },
  {
    "id": "core-009",
    "name": "프로베니우스 노름 상관계수 충실도 (Frobenius Correlation Fidelity)",
    "description": "원본 데이터셋의 다변량 상관계수 행렬(Correlation Matrix R_orig)과 합성 데이터셋의 상관계수 행렬(R_synth) 간의 원소별 오차 거리를 프로베니우스 노름(Frobenius Norm)으로 정량 계산한 종합 품질 점수입니다. 단일 변수의 분포뿐만 아니라 수십 개 변수 간의 복잡한 교차 상관관계(예: 복용량과 혈중 농도 간 상관성)가 합성 과정에서 깨지지 않고 얼마나 완벽히 유지되었는지를 0~100점 척도로 평가합니다.",
    "url": "",
    "category": "머신러닝 & 딥러닝",
    "initial": "ㅍ"
  },
  {
    "id": "core-010",
    "name": "Kiwi 기반 C++ 형태소 분석 PII 탐지 (Kiwi PII Detector)",
    "description": "대규모 자바 가상머신(JVM) 설치 없이 순수 C++ 네이티브 바이너리로 구동되는 초고속 한국어 형태소 분석 엔진(kiwipiepy)을 결합한 지능형 개인정보 탐지 기술입니다. 정규식만으로는 감지하기 어려운 비정형 서술형 문장 속의 인명(NNP), 직책·호칭 결합형 성명, 병원·약국·제약사 등 기관명, 행정구역 지명을 문맥 품사 분석을 통해 95% 이상의 정확도로 자동 분리 추출하여 가명처리 누락을 원천 차단합니다.",
    "url": "",
    "category": "보안 & 프라이버시",
    "initial": "K"
  },
  {
    "id": "core-011",
    "name": "문자 오차율 (CER, Character Error Rate) & RapidFuzz",
    "description": "광학 문자 인식(OCR) 엔진이 판독한 텍스트 결과와 실제 표준 정답 텍스트 간의 오차 비율을 레벤슈타인 편집 거리(삽입, 삭제, 대체 연산 횟수) 기반으로 측정하는 표준 지표입니다. CER = (삽입 + 삭제 + 대체) / 전체 정답 문자수로 계산되며, 0에 가까울수록 완벽한 인식률을 의미합니다. C++ 가속 rapidfuzz를 통해 수천 건의 의약품·행정 용어와 실시간 퍼지 매칭하여 오인식된 단어를 표준 사전 단어로 자동 보정합니다.",
    "url": "",
    "category": "자연어 & 멀티모달",
    "initial": "ㅁ"
  },
  {
    "id": "core-012",
    "name": "스캔 문서 미세 기울기 자동 보정 (Deskewing)",
    "description": "모바일 카메라 촬영이나 평판 스캐너 급지 과정에서 불가피하게 발생하는 문서의 미세 회전 각도(0.1°~45°)를 자동 보정하는 이미지 전처리 알고리즘입니다. OpenCV 이진화 및 라돈 변환/모폴로지 최소 면적 경계 사각형(MinAreaRect)을 통해 텍스트 행의 주축 기울기 각도를 계산하고, 아핀(Affine) 변환으로 무손실 수평 회전 정렬함으로써 OCR 텍스트 추출률과 표 격자 인식률을 극대화합니다.",
    "url": "",
    "category": "자연어 & 멀티모달",
    "initial": "ㅅ"
  },
  {
    "id": "core-013",
    "name": "모폴로지 기반 표 복원 (Morphology Table Reconstruction)",
    "description": "스캔 PDF 및 이미지 문서에 포함된 인쇄 표를 컴퓨터 비전 모폴로지(Morphology) 연산으로 분석하는 기술입니다. 가로선 추출 커널과 세로선 추출 커널을 교차 연산하여 표의 외곽선, 셀 테두리, 병합 셀 좌표를 검출하고, 내부 OCR 텍스트 블록의 바운딩 박스를 좌표 기반으로 각 셀에 정확히 매핑하여 HWPX/DOCX 규격의 네이티브 표(Table)로 1:1 완벽 재현합니다.",
    "url": "",
    "category": "자연어 & 멀티모달",
    "initial": "ㅁ"
  },
  {
    "id": "core-014",
    "name": "인메모리 임베디드 OLAP 엔진 (DuckDB In-Memory OLAP)",
    "description": "대용량 데이터 분석에 최적화된 고성능 컬럼 지향(Columnar-vectorized) 임베디드 SQL 엔진입니다. 외부 데이터베이스 서버 구동이나 네트워크 I/O 없이 로컬 프로세스 메모리 내에서 수백만 행의 CSV, Parquet, SQLite 데이터를 초고속(수십 ms 단위)으로 스캔하여 결측치, 통계 요약(최소/최대/평균/표준편차), 고유값 수 등을 즉시 프로파일링합니다.",
    "url": "",
    "category": "머신러닝 & 딥러닝",
    "initial": "ㅇ"
  },
  {
    "id": "core-015",
    "name": "HWPX 개방형 워드프로세서 규격 (OWPML / KS X 6101)",
    "description": "한글과컴퓨터의 기존 폐쇄형 바이너리 포맷(HWP 5.0)을 대체하기 위해 제정된 국가 표준(KS X 6101) 개방형 문서 형식입니다. ZIP 아카이브 내부에 XML 스키마(OWPML)로 본문 문단(section0.xml), 표 구조, 스타일, 이미지 바이너리를 체계적으로 패키징하여, 한글 오피스 설치 없이도 프로그래밍 방식으로 문서 내용을 안전하게 읽고 쓰고 가명처리할 수 있습니다.",
    "url": "",
    "category": "자연어 & 멀티모달",
    "initial": "H"
  },
  {
    "id": "core-016",
    "name": "레거시 HWP 5.0 OLE 바이너리 복원 (Legacy OLE Compound Document)",
    "description": "2002년부터 수십 년간 널리 사용되어 온 마이크로소프트 CFB(Compound File Binary) 구조 기반 구버전 HWP 문서 포맷입니다. 문서 내부의 BodyText/Section 바이너리 스트림과 Zlib/Deflate 압축 레코드를 직접 파싱하여, 구형 문서의 표 서식과 문단 레이아웃을 손상 없이 추출하고 최신 표준 HWPX 및 DOCX로 무손실 고품질 변환을 수행합니다.",
    "url": "",
    "category": "자연어 & 멀티모달",
    "initial": "ㄹ"
  },
  {
    "id": "core-017",
    "name": "K-익명성 (k-Anonymity)",
    "description": "데이터셋 내에 포함된 준식별자(Quasi-identifier: 나이, 성별, 지역, 직업 등) 속성의 조합이 동일한 레코드가 최소 k개 이상 존재하도록 보장하는 고전적 비식별화 기법입니다. 공격자가 보조 정보를 이용해 특정 개인을 1/k 이하의 확률로만 유추할 수 있도록 레코드를 범주화(Generalization)하거나 국소 마스킹(Suppression)을 적용합니다.",
    "url": "",
    "category": "보안 & 프라이버시",
    "initial": "K"
  },
  {
    "id": "core-018",
    "name": "L-다양성 (l-Diversity)",
    "description": "K-익명성을 만족하더라도 동일 집단 내의 민감 속성(예: 특정 희귀 질병)이 모두 동일한 경우 발생하는 동질성 공격(Homogeneity Attack)을 방어하기 위한 프라이버시 모델입니다. 동일한 준식별자 조합을 공유하는 각 동질 집단 내에서 민감 속성의 고유값 종류가 최소 l개 이상 다양하게 분포하도록 강제합니다.",
    "url": "",
    "category": "보안 & 프라이버시",
    "initial": "L"
  },
  {
    "id": "core-019",
    "name": "T-근접성 (t-Closeness)",
    "description": "L-다양성을 적용하더라도 민감 속성의 분포가 전체 모집단의 통계적 분포와 크게 왜곡될 때 발생하는 쏠림 공격(Skewness Attack)을 방어하는 기법입니다. 각 동질 집단 내의 민감 속성 확률 분포와 전체 데이터셋의 민감 속성 확률 분포 간의 거리(Earth Mover's Distance)가 사전에 정의된 임계값 t 이하가 되도록 유지합니다.",
    "url": "",
    "category": "보안 & 프라이버시",
    "initial": "T"
  },
  {
    "id": "core-020",
    "name": "가명정보 결합 및 결합전문기관 (Pseudonymized Data Combination)",
    "description": "대한민국 개인정보보호법 제28조의3에 의거하여, 통계작성, 과학적 연구, 공익적 기록보존 목적으로 서로 다른 개인정보처리자가 보유한 가명정보를 안전하게 결합하는 제도입니다. 개인정보보호위원회 또는 관계 중앙행정기관이 지정한 공인 결합전문기관에서 결합키(Salted Hash) 연계를 수행하고, 반출심사위원회의 승인을 거쳐 재식별 위험이 없는 안전한 상태로 반출됩니다.",
    "url": "",
    "category": "보안 & 프라이버시",
    "initial": "ㄱ"
  },
  {
    "id": "core-021",
    "name": "가명화 토큰 금고 (Token Vault & Reversible Pseudonymization)",
    "description": "주민등록번호, 환자등록번호, 전화번호 등 고유식별자를 무작위 난수형 대체 토큰(Token)으로 변환할 때, 원본 식별자와 토큰 간의 1:1 매핑 정보를 물리적·암호학적으로 격리된 보안 저장소에 보관하는 체계입니다. 데이터 분석 환경에는 토큰화된 데이터만 제공되며, 법적 인가나 임상 추적 조사 등 정당한 권한이 부여된 경우에만 엄격한 접근 통제 하에 복원이 허용됩니다.",
    "url": "",
    "category": "보안 & 프라이버시",
    "initial": "ㄱ"
  },
  {
    "id": "core-022",
    "name": "동형암호 (Homomorphic Encryption, HE)",
    "description": "데이터를 복호화하여 평문으로 되돌리지 않고도, 암호문 상태 그대로 덧셈과 곱셈 등 산술 연산 및 머신러닝 추론을 수행할 수 있는 차세대 암호 기술입니다. 연산 결과 또한 암호문으로 도출되며 오직 비밀키 보유자만이 최종 결과를 복호화할 수 있으므로, 제3자 클라우드나 외부 분석 기관에 의료·식약처 민감 데이터를 위탁 처리할 때 원천적인 데이터 유출을 방지합니다.",
    "url": "",
    "category": "보안 & 프라이버시",
    "initial": "ㄷ"
  },
  {
    "id": "core-023",
    "name": "데이터 드리프트 및 공변량 변화 (Data Drift & Covariate Shift)",
    "description": "인공지능 모델의 학습 시점에 사용된 원본 데이터의 통계적 분포와 실제 운영 환경(Inference)에서 인입되는 데이터의 분포가 시간 경과나 외부 환경 요인으로 인해 점진적 또는 급격하게 달라지는 현상입니다. 합성 데이터 생성기에서는 주기적으로 원본-합성 데이터 간의 KS 검정 및 상관 행렬을 재평가하여 드리프트를 사전에 감지하고 모델을 재학습합니다.",
    "url": "",
    "category": "머신러닝 & 딥러닝",
    "initial": "ㄷ"
  },
  {
    "id": "core-024",
    "name": "합성데이터 다변량 연계 충실도 (Relational Synthesis Parity)",
    "description": "환자 기본 테이블(1)과 처방/진료 내역 테이블(N)처럼 외래키(Foreign Key)로 연결된 다중 테이블(Multi-table) 관계형 데이터베이스 환경에서, 개별 테이블의 단변량 분포뿐만 아니라 부모-자식 간의 참조 무결성(Referential Integrity)과 집계 통계(환자당 처방 건수 분포 등)를 보존하면서 동시에 합성하는 고급 데이터 엔지니어링 기술입니다.",
    "url": "",
    "category": "머신러닝 & 딥러닝",
    "initial": "ㅎ"
  },
  {
    "id": "core-025",
    "name": "단일식별 위험 (Singling-out Risk, Anonymeter)",
    "description": "익명화 또는 합성 데이터셋 내에서 공격자가 외부 보조 정보를 결합하여 특정 레코드가 현실 세계의 단 한 명의 개인을 고유하게 가리킨다는 사실을 분리·특정해낼 수 있는 확률을 통계적으로 정량화한 지표입니다 (EU GDPR 제29조 표준).",
    "url": "",
    "category": "보안 & 프라이버시",
    "initial": "ㄷ"
  },
  {
    "id": "core-026",
    "name": "연결성 위험 (Linkability Risk, Anonymeter)",
    "description": "두 개 이상의 서로 다른 공개 데이터셋 또는 비공개 데이터 소스에서 동일 인물에 속하는 레코드들을 연결키나 공통 준식별자 패턴을 통해 연계해낼 수 있는 취약성 확률을 평가하는 프라이버시 지표입니다.",
    "url": "",
    "category": "보안 & 프라이버시",
    "initial": "ㅇ"
  },
  {
    "id": "core-027",
    "name": "속성추론 위험 (Inference Risk, Anonymeter)",
    "description": "공격자가 특정 대상자에 대해 이미 알려진 일부 준식별자 정보(예: 연령, 거주지)를 바탕으로, 해당 대상자의 숨겨진 민감한 속성(예: 특정 희귀 질환 진단명, 소득 분위, 투약 이력)을 기계학습 모델을 통해 높은 정확도로 예측해낼 수 있는 위험도를 정량화한 척도입니다.",
    "url": "",
    "category": "보안 & 프라이버시",
    "initial": "ㅅ"
  },
  {
    "id": "core-028",
    "name": "재현 데이터셋 유용성 평가 (Utility Metrics)",
    "description": "합성 데이터가 원본 데이터의 통계적 특성, 머신러닝 학습 모델 성능(TSTR: Train on Synthetic, Test on Real), 변수 간 상호 의존성을 얼마나 잘 보존하고 있는지를 측정하는 다차원 정량 평가 체계입니다.",
    "url": "",
    "category": "머신러닝 & 딥러닝",
    "initial": "ㅈ"
  },
  {
    "id": "core-029",
    "name": "퍼지 사전 매칭 (Fuzzy Dictionary Matching)",
    "description": "OCR 인식 오류나 사용자 오탈자가 포함된 텍스트를 표준 의료/의약품/행정 사전 데이터와 레벤슈타인 편집 거리 및 토큰 정렬 유사도로 비교하여 가장 확률이 높은 표준 정식 용어로 자동 교정하는 지능형 알고리즘입니다.",
    "url": "",
    "category": "자연어 & 멀티모달",
    "initial": "ㅍ"
  },
  {
    "id": "core-030",
    "name": "합성데이터 출처 증명 및 워터마킹 (Data Provenance & Watermarking)",
    "description": "생성된 데이터셋이 실제 개인의 원본 정보가 아닌 알고리즘에 의해 생성된 인공 합성 데이터임을 증명하고, 데이터의 생성 일시, 적용된 모델 버전, 사용된 난수 시드(Seed) 및 라이선스를 암호학적 메타데이터로 기록·보증하는 기술입니다.",
    "url": "",
    "category": "보안 & 프라이버시",
    "initial": "ㅎ"
  },
  ...
  },
  {
    "id": "glossary-017",
    "name": "3D 재구성",
    "description": "3D 재구성에 대해.: 이 첨단 프로세스가 실제 객체나 환경을 어떻게 포착하여 포토그래메트리, 레이저 스캐닝, AI 기반 알고리즘 등의 기술을 활용해 정교한 3D 모델로 변환하는지. 주요 개념, 활용 사례, 도전 과제, 미래 동향을.",
    "url": "",
    "category": "자연어 & 멀티모달",
    "initial": "0-9"
  },
  ...
  },
  {
    "id": "glossary-019",
    "name": "AI SDR",
    "description": "AI SDR가 무엇인지, 인공지능 세일즈 개발 담당자가 어떻게 영업팀의 생산성과 효율성을 높이기 위해 잠재고객 발굴, 리드 자격 심사, 아웃리치, 팔로우업을 자동화하는지.",
    "url": "",
  ...
  },
  {
    ...
    "description": "머신러닝에서 AI 모델의 정확도와 안정성의 중요성을. 이러한 지표가 사기 탐지, 의료 진단, 챗봇과 같은 애플리케이션에 어떤 영향을 미치는지 배우고, 신뢰할 수 있는 AI 성능을 높이는 기법을 탐구해보세요.",
    "url": "",
    "category": "비즈니스 AI & 자동화",
    "initial": "A"
  },
  ...
  },
  {
    "id": "glossary-033",
    "name": "AI 시스템 엔지니어",
    "description": "AI 시스템 엔지니어의 역할을.: AI 시스템 설계, 개발, 유지보수, 머신러닝 통합, 인프라 관리, 비즈니스의 AI 자동화 주도.",
    "url": "",
    "category": "비즈니스 AI & 자동화",
    "initial": "A"
  },
  ...
  },
  {
    ...
    "description": "AI 아트에서 시드가 무엇인지, 이미지 생성 과정에 어떻게 영향을 미치는지, 그리고 아티스트들이 생성 예술 플랫폼에서 일관성 또는 창의적 탐구를 위해 시드를 어떻게 사용하는지.",
    "url": "",
    "category": "자연어 & 멀티모달",
    "initial": "A"
  },
  ...
  },
  {
    "id": "glossary-037",
    "name": "AI 윤리",
    "description": "AI 윤리 가이드라인을 살펴보세요: AI 기술의 윤리적 개발, 배포 및 사용을 보장하는 원칙과 프레임워크를 안내합니다. 공정성, 투명성, 책임성, 글로벌 기준, 그리고 책임 있는 AI를 위한 전략에 대해.",
    "url": "",
    "category": "머신러닝 & 딥러닝",
    "initial": "A"
  },
  ...
  },
  {
    "id": "glossary-040",
    "name": "AI 컨설턴트",
    "description": "AI 컨설턴트는 AI 기술과 비즈니스 전략을 연결하여, 기업이 혁신, 효율성 및 성장을 이끌 수 있도록 AI 도입을 안내합니다. 그들의 역할, 책임, 필요한 역량, 그리고 AI 컨설팅이 비즈니스를 어떻게 변화시키는지.",
    "url": "",
    "category": "비즈니스 AI & 자동화",
    "initial": "A"
  },
  {
    "id": "glossary-041",
    "name": "AI 콘텐츠 제작",
    "description": "AI 콘텐츠 제작은 인공지능을 활용하여 텍스트, 비주얼, 오디오 등 디지털 콘텐츠의 생성, 선별, 개인화를 자동화하고 향상시킵니다. 도구, 이점, 단계별 가이드를 통해 효율적이고 확장 가능한 콘텐츠 워크플로우를.",
    "url": "",
    "category": "AI 에이전트 & RAG",
    "initial": "A"
  },
  {
    "id": "glossary-042",
    "name": "AI 투명성",
    "description": "AI 투명성은 인공지능 시스템의 작동 방식과 의사결정 과정을 이해관계자에게 이해할 수 있도록 만드는 실천입니다. 그 중요성, 핵심 요소, 규제 프레임워크, 구현 기법, 과제, 그리고 실제 활용 사례를.",
    "url": "",
    "category": "머신러닝 & 딥러닝",
    "initial": "A"
  },
  ...
  },
  {
    "id": "glossary-044",
    "name": "AI 파트너십",
    "description": "대학교와 민간 기업 간의 AI 파트너십이 어떻게 학문적 지식과 산업적 응용을 결합하여 혁신, 연구, 그리고 역량 개발을 이끄는지. 성공적인 협업 사례, 주요 특징, 이점 및 도전과제에 대해 소개합니다.",
    "url": "",
    "category": "머신러닝 & 딥러닝",
    "initial": "A"
  },
  ...
  },
  {
    "id": "glossary-046",
    "name": "AI 프로토타입 개발",
    "description": "AI 프로토타입 개발은 AI 시스템의 초기 버전을 설계하고 만드는 반복적인 과정으로, 본격적인 대량 생산 전에 실험, 검증, 자원 최적화를 가능하게 합니다. 주요 라이브러리, 접근 방식, 다양한 산업에서의 활용 사례를.",
    "url": "",
    "category": "머신러닝 & 딥러닝",
    "initial": "A"
  },
  {
    "id": "glossary-047",
    "name": "AI와 인권",
    "description": "인공지능이 인권에 미치는 영향에 대해 탐구해 보세요. 서비스 접근성 향상과 같은 이점과 프라이버시 침해, 편향 등과 같은 위험 사이의 균형을. 국제적 기준, 규제적 과제, 그리고 기본권 보호를 위한 책임 있는 AI 도입의 중요성에 대해 학습할 수 있습니다.",
    "url": "",
    "category": "보안 & 프라이버시",
    "initial": "A"
  },
  ...
  },
  {
    "id": "glossary-052",
    "name": "BERT",
    "description": "BERT(양방향 인코더 표현 변환기)는 Google에서 개발한 오픈 소스 기계 학습 프레임워크로, 자연어 처리를 위한 혁신적인 기술입니다. BERT의 양방향 Transformer 아키텍처가 AI 언어 이해를 혁신한 방식과 NLP, 챗봇, 자동화 및 주요 연구 발전에 어떻게 활용되는지.",
    "url": "",
    "category": "자연어 & 멀티모달",
    "initial": "B"
  },
  ...
  },
  {
    "id": "glossary-059",
    "name": "Claude 3.5 소네트",
    "description": "Anthropic의 Claude 3.5 소네트에 대해 자세히.: 다른 모델과의 비교, 강점과 약점, 추론, 코딩, 비주얼 작업 등 다양한 분야에서의 활용 사례를 확인할 수 있습니다.",
    "url": "",
    "category": "머신러닝 & 딥러닝",
    "initial": "C"
  },
  ...
  },
  {
    "id": "glossary-064",
    "name": "Crew AI",
    "description": "Crew AI에 대한 기본 정보를. 주요 기능, 장단점, 그리고 대안에 대한 빠른 개요를 제공합니다.",
    "url": "",
    "category": "머신러닝 & 딥러닝",
    "initial": "C"
  },
  ...
  },
  {
    "id": "glossary-082",
    "name": "LIX 가독성 측정법",
    "description": "LIX 가독성 측정법에 대해. 이 공식은 문장의 길이와 긴 단어를 분석하여 텍스트의 복잡성을 평가하도록 개발되었습니다. 교육, 출판, 저널리즘, AI 등 다양한 분야에서의 활용 사례를 이해할 수 있습니다.",
    "url": "",
    "category": "자연어 & 멀티모달",
    "initial": "L"
  },
  ...
  },
  {
    "id": "glossary-096",
    "name": "Rytr",
    "description": "Rytr에 대한 기본 정보를. 주요 기능, 장단점, 대안에 대한 빠른 개요입니다.",
    "url": "",
    "category": "머신러닝 & 딥러닝",
    "initial": "R"
  },
  ...
  },
  {
    "id": "glossary-104",
    "name": "xAI의 Grok",
    "description": "Elon Musk가 이끄는 xAI의 첨단 AI 챗봇 Grok 모델에 대해 자세히. 실시간 데이터 접근, 주요 기능, 벤치마크, 활용 사례 및 다른 AI 모델과의 비교를 확인할 수 있습니다.",
    "url": "",
    "category": "비즈니스 AI & 자동화",
    "initial": "X"
  },
  ...
  },
  {
    "id": "glossary-106",
    "name": "가독성",
    "description": "가독성은 독자가 작성된 텍스트를 얼마나 쉽게 이해할 수 있는지를 측정하며, 어휘, 문장 구조, 조직을 통한 명확성과 접근성을 반영합니다. 교육, 마케팅, 의료 등 다양한 분야에서 가독성의 중요성, 측정 공식, AI 도구가 가독성 향상에 어떻게 기여하는지.",
    "url": "",
    "category": "자연어 & 멀티모달",
    "initial": "ㄱ"
  },
  {
    "id": "glossary-107",
    "name": "감정 분석",
    "description": "감정 분석(오피니언 마이닝)은 텍스트의 감정적 톤을 긍정, 부정, 중립으로 분류하고 해석하는 데 중요한 AI 및 자연어 처리(NLP) 작업입니다. 그 중요성, 유형, 접근 방식, 그리고 비즈니스에서의 실질적 활용 사례를.",
    "url": "",
    "category": "자연어 & 멀티모달",
    "initial": "ㄱ"
  },
  ...
  },
  {
    "id": "glossary-113",
    "name": "검색 파이프라인",
    "description": "챗봇을 위한 검색 파이프라인이 무엇인지, 그 구성 요소와 활용 사례, 그리고 RAG 및 외부 데이터 소스가 어떻게 정확하고 맥락을 고려한 실시간 응답을 가능하게 하는지.",
    "url": "",
    "category": "AI 에이전트 & RAG",
    "initial": "ㄱ"
  },
  ...
  },
  {
    ...
    "description": "과적합은 인공지능(AI)과 머신러닝(ML)에서 매우 중요한 개념으로, 모델이 학습 데이터를 지나치게 학습하여 잡음까지 포함하게 되어 새로운 데이터에 대해 일반화 성능이 떨어지는 현상을 말합니다. 과적합을 식별하고 효과적으로 방지하는 다양한 기법을.",
    "url": "",
    "category": "머신러닝 & 딥러닝",
    "initial": "ㄱ"
  },
  {
    "id": "glossary-121",
    "name": "광학 문자 인식(OCR)",
    "description": "광학 문자 인식(OCR)은 스캔된 문서, PDF 또는 이미지를 편집 가능하고 검색 가능한 데이터로 변환하는 혁신적인 기술입니다. OCR의 작동 원리, 종류, 응용 분야, 장점, 한계, 그리고 AI 기반 OCR 시스템의 최신 발전에 대해.",
    "url": "",
    "category": "자연어 & 멀티모달",
    "initial": "ㄱ"
  },
  ...
  },
  {
    "id": "glossary-126",
    "name": "구현된 AI 에이전트",
    "description": "구현된 AI 에이전트는 물리적 또는 가상 몸체를 통해 환경을 인지하고 해석하며 상호작용하는 지능형 시스템입니다. 이러한 에이전트가 로보틱스와 디지털 시뮬레이션에서 어떻게 동작하며, 인지, 추론, 행동이 요구되는 작업을 수행하는지.",
    "url": "",
    "category": "AI 에이전트 & RAG",
    "initial": "ㄱ"
  },
  {
    "id": "glossary-127",
    "name": "군집화",
    "description": "군집화는 비지도 학습 기법으로, 유사한 데이터 포인트들을 함께 묶어 레이블이 없는 데이터에서도 탐색적 데이터 분석을 가능하게 합니다. 군집화의 유형, 활용 분야, 임베딩 모델이 군집화에 어떻게 기여하는지.",
    "url": "",
    "category": "자연어 & 멀티모달",
    "initial": "ㄱ"
  },
  ...
  },
  {
    "id": "glossary-136",
    "name": "달-이(DALL-E)",
    "description": "DALL-E는 OpenAI가 개발한 텍스트-이미지 생성 AI 모델 시리즈로, 딥러닝을 활용해 텍스트 설명으로부터 디지털 이미지를 생성합니다. 역사, 예술·마케팅·교육 분야 활용, 윤리적 고려 사항 등에 대해.",
    "url": "",
    "category": "자연어 & 멀티모달",
    "initial": "ㄷ"
  },
  ...
  },
  {
    "id": "glossary-145",
    "name": "데이터 부족",
    "description": "데이터 부족은 머신러닝 모델 학습이나 종합적인 분석에 충분한 데이터가 없어 정확한 AI 시스템 개발을 저해하는 현상입니다. 데이터 부족의 원인, 영향, 그리고 AI 및 자동화에서 이를 극복하는 기술을.",
    "url": "",
    "category": "비즈니스 AI & 자동화",
    "initial": "ㄷ"
  },
  {
    "id": "glossary-146",
    "name": "데이터 정제",
    "description": "데이터 정제는 데이터의 오류나 불일치 사항을 탐지하고 수정하는 중요한 과정으로, 데이터의 품질을 높여 분석 및 의사결정을 위한 정확성, 일관성, 신뢰성을 보장합니다. 주요 프로세스, 과제, 도구, 그리고 효과적인 데이터 정제에서 AI와 자동화의 역할을.",
    "url": "",
    "category": "비즈니스 AI & 자동화",
    "initial": "ㄷ"
  },
  ...
  },
  {
    "id": "glossary-153",
    "name": "라이트소닉",
    "description": "Writesonic에 대한 기본 정보를. 주요 기능, 장단점, 그리고 대안에 대한 빠른 개요를 제공합니다.",
    "url": "",
    "category": "머신러닝 & 딥러닝",
    "initial": "ㄹ"
  },
  ...
  },
  {
    "id": "glossary-158",
    "name": "리드 라우팅",
    "description": "리드 라우팅은 잠재 고객을 위치, 제품 관심사, 전문성 등과 같은 기준에 따라 적합한 영업 담당자에게 자동으로 배정하여, 가장 알맞은 담당자와 연결해주는 프로세스입니다. 자동화와 AI가 리드 분배를 어떻게 최적화하여 전환율과 고객 경험을 높이는지.",
    "url": "",
    "category": "비즈니스 AI & 자동화",
    "initial": "ㄹ"
  },
  ...
  },
  {
    ...
    "description": "머신러닝에서의 리콜(Recall)에 대해. 분류 작업에서 모델 성능을 평가하는 데 중요한 이 지표는 양성 인스턴스를 올바르게 식별하는 것이 얼마나 중요한지 설명합니다. 정의, 계산 방법, 중요성, 활용 사례, 개선 전략까지 모두.",
    "url": "",
    "category": "머신러닝 & 딥러닝",
    "initial": "ㅁ"
  },
  ...
  },
  {
    "id": "glossary-168",
    "name": "모델 드리프트",
    "description": "모델 드리프트(또는 모델 붕괴)는 실제 환경의 변화로 인해 머신러닝 모델의 예측 성능이 시간이 지남에 따라 저하되는 현상을 의미합니다. AI 및 머신러닝에서 모델 드리프트의 유형, 원인, 탐지 방법, 해결책에 대해.",
    "url": "",
    "category": "머신러닝 & 딥러닝",
    "initial": "ㅁ"
  },
  ...
  },
  {
    "id": "glossary-173",
    "name": "문단 리라이팅 도구",
    "description": "문단 리라이팅 도구가 무엇인지, 어떻게 작동하는지, 주요 기능과 고급 언어 처리 기술을 통해 글쓰기 품질 향상, 표절 방지, SEO 강화에 어떻게 기여하는지.",
    "url": "",
    "category": "머신러닝 & 딥러닝",
    "initial": "ㅁ"
  },
  ...
  },
  {
    "id": "glossary-176",
    "name": "문장 리라이팅 도구",
    "description": "AI 문장 리라이팅 도구가 무엇인지, 어떻게 작동하는지, 사용 사례는 무엇이며, 작가, 학생, 마케터가 의미를 보존하면서도 명확성을 높여 텍스트를 변환하는 데 어떻게 도움이 되는지.",
    "url": "",
    "category": "자연어 & 멀티모달",
    "initial": "ㅁ"
  },
  {
    "id": "glossary-177",
    "name": "미스트랄 AI",
    "description": "미스트랄 AI와 그들이 제공하는 LLM 모델에 대해. 이러한 모델이 어떻게 사용되는지, 그리고 무엇이 이들을 특별하게 만드는지.",
    "url": "",
    "category": "LLM & 생성형 AI",
    "initial": "ㅁ"
  },
  ...
  },
  {
    "id": "glossary-182",
    "name": "버티컬 AI 에이전트",
    "description": "버티컬 AI 에이전트는 특정 산업의 고유한 과제를 해결하고 프로세스를 최적화하기 위해 설계된 산업 특화 인공지능 솔루션입니다. 버티컬 AI 에이전트가 전문적이고 높은 임팩트를 지닌 응용 프로그램으로 엔터프라이즈 소프트웨어를 어떻게 혁신하는지.",
    "url": "",
    "category": "AI 에이전트 & RAG",
    "initial": "ㅂ"
  },
  ...
  },
  {
    "id": "glossary-187",
    "name": "부스팅",
    "description": "부스팅은 여러 개의 약한 학습자의 예측을 결합하여 강한 학습자를 만드는 머신러닝 기법으로, 정확도를 향상시키고 복잡한 데이터를 처리합니다. 주요 알고리즘, 장점, 도전 과제, 실제 적용 사례를.",
    "url": "",
    "category": "머신러닝 & 딥러닝",
    "initial": "ㅂ"
  },
  ...
  },
  {
    "id": "glossary-189",
    "name": "비정형 데이터",
    "description": "비정형 데이터가 무엇인지, 구조화된 데이터와 어떻게 다른지. 비정형 데이터의 과제와 활용되는 도구에 대해 배웁니다.",
    "url": "",
    "category": "머신러닝 & 딥러닝",
    "initial": "ㅂ"
  },
  ...
  },
  {
    "id": "glossary-205",
    "name": "시퀀스 모델링",
    "description": "AI와 머신러닝에서 시퀀스 모델링을.—RNN, LSTM, GRU, 트랜스포머를 이용해 텍스트, 오디오, DNA와 같은 데이터 시퀀스를 예측하고 생성합니다. 핵심 개념, 응용 분야, 과제, 최신 연구 동향을.",
    "url": "",
    "category": "자연어 & 멀티모달",
    "initial": "ㅅ"
  },
  {
    "id": "glossary-206",
    "name": "식별 모델",
    "description": "식별 AI 모델에 대해.—클래스 간의 결정 경계를 모델링함으로써 분류와 회귀에 집중하는 머신러닝 모델입니다. 동작 방식, 장점, 과제, 그리고 NLP, 컴퓨터 비전, AI 자동화에서의 적용 사례를 이해할 수 있습니다.",
    "url": "",
    "category": "자연어 & 멀티모달",
    "initial": "ㅅ"
  },
  ...
  },
  {
    ...
    "description": "Garbage In, Garbage Out(GIGO)는 AI 및 기타 시스템의 출력 품질이 입력 품질에 직접적으로 달려 있음을 강조합니다. AI에서의 의미, 데이터 품질의 중요성, 더 정확하고 공정하며 신뢰할 수 있는 결과를 위해 GIGO를 완화하는 전략에 대해.",
    "url": "",
    "category": "머신러닝 & 딥러닝",
    "initial": "ㅅ"
  },
  ...
  },
  {
    ...
    "description": "앤트로픽의 Claude에 대해 자세히. 사용 목적, 제공되는 다양한 모델, 고유한 기능을 이해할 수 있습니다.",
    "url": "",
    "category": "LLM & 생성형 AI",
    "initial": "ㅇ"
  },
  ...
  },
  {
    "id": "glossary-214",
    "name": "양자 컴퓨팅",
    "description": "양자 컴퓨팅이 무엇인지 쉽고 간단하게. 어떻게 활용되는지, 어떤 도전과제와 미래의 희망이 있는지 확인할 수 있습니다.",
    "url": "",
    "category": "머신러닝 & 딥러닝",
    "initial": "ㅇ"
  },
  ...
  },
  {
    ...
    "description": "역전파는 인공 신경망의 예측 오류를 최소화하기 위해 가중치를 조정하여 학습시키는 알고리즘입니다. 작동 원리, 단계, 신경망 학습에서의 원칙을.",
    "url": "",
    "category": "머신러닝 & 딥러닝",
    "initial": "ㅇ"
  },
  ...
  },
  {
    "id": "glossary-222",
    "name": "영업 스크립트 생성기",
    "description": "AI 영업 스크립트 생성기가 NLP와 NLG를 활용해 통화, 이메일, 영상, 소셜 아웃리치에 맞춘 맞춤형·설득력 있는 영업 스크립트를 제작하며, 영업 커뮤니케이션을 간소화하고 전환율을 높이는 방법을.",
    "url": "",
    "category": "자연어 & 멀티모달",
    "initial": "ㅇ"
  },
  ...
  },
  {
    "id": "glossary-224",
    "name": "예측 분석",
    "description": "AI 기반 예측 분석 기술, 작동 원리, 그리고 다양한 산업에 미치는 이점에 대해 자세히.",
    "url": "",
    "category": "머신러닝 & 딥러닝",
    "initial": "ㅇ"
  },
  ...
  },
  {
    "id": "glossary-242",
    "name": "이미지 인식",
    "description": "AI에서 이미지 인식이 무엇인지. 어떤 용도로 사용되는지, 최신 트렌드는 무엇인지, 유사 기술과 어떻게 다른지 설명합니다.",
    "url": "",
    "category": "자연어 & 멀티모달",
    "initial": "ㅇ"
  },
  {
    "id": "glossary-243",
    "name": "이미지에서의 이상 탐지",
    "description": "이미지에서의 이상 탐지는 정상에서 벗어난 패턴을 식별하여 산업 검사나 의료 영상과 같은 분야에서 중요하게 사용됩니다. 비지도 및 약지도 방법, AI 통합, 실제 활용 사례에 대해.",
    "url": "",
    "category": "자연어 & 멀티모달",
    "initial": "ㅇ"
  },
  ...
  },
  {
    "id": "glossary-248",
    "name": "인공지능 초지능(ASI)",
    "description": "인공지능 초지능(ASI)은 모든 영역에서 인간 지능을 능가하는 이론적 AI로, 자기 개선과 다중 모달 능력을 갖추고 있습니다. 그 특징, 구성 요소, 응용 분야, 이점, 윤리적 위험성에 대해.",
    "url": "",
    "category": "보안 & 프라이버시",
    "initial": "ㅇ"
  },
  {
    ...
    "description": "인공지능 투자 수익률(ROAI)은 AI 투자가 기업의 운영, 생산성, 수익성에 미치는 영향을 측정합니다. 전략, 실무 사례, 연구 인사이트를 통해 AI 프로젝트의 수익을 평가, 측정, 극대화하는 방법을.",
    "url": "",
    "category": "비즈니스 AI & 자동화",
    "initial": "ㅇ"
  },
  ...
  },
  {
    "id": "glossary-251",
    "name": "인사이트 엔진",
    "description": "인사이트 엔진이란 무엇인지.—AI 기반의 고급 플랫폼으로, 컨텍스트와 의도를 이해하여 데이터 검색과 분석을 향상시킵니다. 인사이트 엔진이 자연어 처리(NLP), 머신러닝, 딥러닝을 어떻게 통합해 구조화 및 비구조화 데이터 소스에서 실행 가능한 인사이트를 제공하는지 확인하세요.",
    "url": "",
    "category": "자연어 & 멀티모달",
    "initial": "ㅇ"
  },
  ...
  },
  {
    "id": "glossary-256",
    "name": "일반화 오류",
    "description": "일반화 오류는 머신러닝 모델이 보지 않은 데이터를 얼마나 잘 예측하는지 측정하며, 편향과 분산의 균형을 맞춰 견고하고 신뢰할 수 있는 AI 응용을 보장합니다. 그 중요성과 수학적 정의, 실제 성공을 위한 효과적인 최소화 기법을.",
    "url": "",
    "category": "머신러닝 & 딥러닝",
    "initial": "ㅇ"
  },
  {
    "id": "glossary-257",
    "name": "읽기 수준",
    "description": "읽기 수준이란 무엇이며, 어떻게 측정되고 왜 중요한지. 다양한 평가 시스템, 읽기 능력에 영향을 주는 요인, 읽기 수준을 높이기 위한 전략과 AI가 개인 맞춤형 학습에서 어떤 역할을 하는지도 함께 학습할 수 있습니다.",
    "url": "",
    "category": "머신러닝 & 딥러닝",
    "initial": "ㅇ"
  },
  {
    "id": "glossary-258",
    "name": "임베딩 벡터",
    "description": "임베딩 벡터는 데이터의 의미적 및 맥락적 관계를 포착하는 다차원 공간에서의 밀집 수치 표현입니다. 임베딩 벡터가 자연어 처리, 이미지 처리, 추천 등 다양한 AI 작업을 어떻게 지원하는지.",
    "url": "",
    "category": "AI 에이전트 & RAG",
    "initial": "ㅇ"
  },
  ...
  },
  {
    "id": "glossary-263",
    "name": "자연어 처리(NLP)",
    "description": "자연어 처리(NLP)는 인공지능(AI)의 한 분야로, 컴퓨터가 인간의 언어를 이해하고 해석하며 생성할 수 있도록 합니다. 주요 개념, 작동 방식, 산업별 응용 사례를.",
    "url": "",
    "category": "자연어 & 멀티모달",
    "initial": "ㅈ"
  },
  {
    "id": "glossary-264",
    "name": "자율주행 차량",
    "description": "AI, 센서, 연결성을 활용하여 인간의 개입 없이 작동하는 자율주행 차량(무인 자동차)에 대해. 핵심 기술, AI의 역할, LLM 통합, 도전 과제, 그리고 스마트 교통의 미래를.",
    "url": "",
    "category": "LLM & 생성형 AI",
    "initial": "ㅈ"
  },
  ...
  },
  {
    "id": "glossary-279",
    "name": "지도 학습",
    "description": "지도 학습은 알고리즘이 레이블이 지정된 데이터를 기반으로 학습하여 새로운, 보지 못한 데이터에 대해 정확한 예측이나 분류를 할 수 있도록 하는 인공지능 및 머신러닝의 기본 개념입니다. 주요 구성 요소, 종류, 그리고 장점에 대해.",
    "url": "",
    "category": "머신러닝 & 딥러닝",
    "initial": "ㅈ"
  },
  ...
  },
  {
    "id": "glossary-287",
    "name": "챗봇",
    "description": "챗봇은 AI와 자연어 처리를 활용해 인간과의 대화를 모방하는 디지털 도구로, 24시간 지원, 확장성, 비용 효율성을 제공합니다. 챗봇의 작동 방식, 유형, 이점, 그리고 플랫폼에서 제공하는 실제 적용 사례를.",
    "url": "",
    "category": "자연어 & 멀티모달",
    "initial": "ㅊ"
  },
  ...
  },
  {
    "id": "glossary-300",
    "name": "클로드 오푸스",
    "description": "Anthropic의 클로드 오푸스 모델에 대해 자세히. 강점과 약점, 그리고 다른 모델과의 비교를 확인할 수 있습니다.",
    "url": "",
    "category": "머신러닝 & 딥러닝",
    "initial": "ㅋ"
  },
  {
    "id": "glossary-301",
    "name": "클로드 하이쿠",
    "description": "Anthropic의 가장 빠르고 저렴한 AI 모델인 Claude Haiku에 대해 자세히. 주요 기능, 엔터프라이즈 활용 사례, Claude 3 패밀리 내 다른 모델과의 비교를 확인할 수 있습니다.",
    "url": "",
    "category": "머신러닝 & 딥러닝",
    "initial": "ㅋ"
  },
  ...
  },
  {
    "id": "glossary-304",
    "name": "텍스트 생성",
    "description": "대형 언어 모델(LLM)을 활용한 텍스트 생성은 머신러닝 모델을 이용해 프롬프트로부터 인간과 유사한 텍스트를 만들어내는 고급 기술을 의미합니다. 트랜스포머 아키텍처로 구동되는 LLM이 콘텐츠 제작, 챗봇, 번역 등 다양한 분야에서 어떻게 혁신을 이끌고 있는지.",
    "url": "",
    "category": "LLM & 생성형 AI",
    "initial": "ㅌ"
  },
  ...
  },
  {
    "id": "glossary-313",
    "name": "특징 엔지니어링 및 추출",
    "description": "특징 엔지니어링과 추출이 원시 데이터를 가치 있는 인사이트로 변환하여 AI 모델의 성능을 어떻게 향상시키는지. 특징 생성, 변환, PCA, 오토인코더 등 주요 기법을 통해 ML 모델의 정확성과 효율성을 높이는 방법을 소개합니다.",
    "url": "",
    "category": "머신러닝 & 딥러닝",
    "initial": "ㅌ"
  },
  {
    "id": "glossary-314",
    "name": "특징 추출",
    "description": "특징 추출은 원시 데이터를 정보가 풍부한 특징의 축소된 집합으로 변환하여, 데이터 단순화, 모델 성능 향상, 연산 비용 절감 등 머신러닝을 강화합니다. 이 포괄적인 가이드에서 기법, 응용, 도구, 과학적 통찰을.",
    "url": "",
    "category": "머신러닝 & 딥러닝",
    "initial": "ㅌ"
  },
  ...
  },
  {
    ...
    "description": "모델 파인튜닝은 사전 학습된 모델을 새로운 작업에 맞게 소폭 조정하여 데이터와 리소스 요구를 줄입니다. 파인튜닝이 전이 학습을 어떻게 활용하는지, 다양한 기법, 모범 사례, 평가 지표를 통해 NLP, 컴퓨터 비전 등에서 모델 성능을 효율적으로 향상하는 방법을.",
    "url": "",
    "category": "LLM & 생성형 AI",
    "initial": "ㅍ"
  },
  ...
  },
  {
    "id": "glossary-329",
    "name": "프레이즈(Frase)",
    "description": "프레이즈(Frase)에 대한 기본 정보를. AI 기반의 SEO 최적화 콘텐츠 제작 도구의 주요 기능, 장단점, 대안들을 확인할 수 있습니다.",
    "url": "",
    "category": "머신러닝 & 딥러닝",
    "initial": "ㅍ"
  },
  {
    "id": "glossary-330",
    "name": "프롬프트",
    "description": "LLM 분야에서 프롬프트란 모델의 출력을 안내하는 입력 텍스트를 의미합니다. 효과적인 프롬프트 작성법, 제로샷·원샷·퓨샷·체인오브쏘트 기법을 통해 AI 언어 모델의 응답 품질을 높이는 방법을.",
    "url": "",
    "category": "LLM & 생성형 AI",
    "initial": "ㅍ"
  },
  {
    "id": "glossary-331",
    "name": "하이퍼파라미터 튜닝",
    "description": "하이퍼파라미터 튜닝은 학습률, 정규화와 같은 파라미터를 조정하여 모델 성능을 최적화하는 머신러닝의 기본 과정입니다. 그리드 서치, 랜덤 서치, 베이지안 최적화 등 다양한 방법을.",
    "url": "",
    "category": "머신러닝 & 딥러닝",
    "initial": "ㅎ"
  },
  {
    "id": "glossary-332",
    "name": "학년 수준",
    "description": "가독성에서 학년 수준이 무엇을 의미하는지, Flesch-Kincaid와 같은 공식으로 어떻게 계산되는지, 그리고 왜 콘텐츠를 독자의 읽기 능력에 맞추는 데 중요한지.",
    "url": "",
    "category": "머신러닝 & 딥러닝",
    "initial": "ㅎ"
  },
  ...
  },
  {
    "id": "glossary-341",
    "name": "협동로봇(코봇, Cobots)",
    "description": "협동로봇(코봇)의 기원, 안전 기능, AI 통합, 다양한 산업 분야에서의 활용, 장점과 한계에 대해. 코봇이 안전한 인간-로봇 상호작용을 가능하게 하고 혁신을 촉진하는 방법을 배울 수 있습니다.",
    "url": "",
    "category": "보안 & 프라이버시",
    "initial": "ㅎ"
  },
  ...
  },
  {
    ...
    "description": "언어 모델에서의 환각은 AI가 그럴듯하게 보이지만 실제로는 부정확하거나 조작된 텍스트를 생성하는 현상입니다. 원인, 탐지 방법, 그리고 AI 결과물에서 환각을 줄이기 위한 전략에 대해.",
    "url": "",
    "category": "LLM & 생성형 AI",
    "initial": "ㅎ"
  },
  {
    "id": "glossary-345",
    "name": "활성화 함수",
    "description": "활성화 함수는 인공 신경망에서 필수적인 요소로, 비선형성을 도입하여 복잡한 패턴 학습을 가능하게 합니다. 이 글에서는 활성화 함수의 목적, 유형, 도전 과제, 그리고 AI, 딥러닝, 신경망에서의 핵심 응용 분야를.",
    "url": "",
    "category": "머신러닝 & 딥러닝",
    "initial": "ㅎ"
  },
